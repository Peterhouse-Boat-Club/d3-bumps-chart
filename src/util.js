...
export function crewColor(name, event = SET.LENTS) {
    ...
    // Remove trailing crew numbers
    const club = name.replace(/[0-9]+$/, '').trim();
    if (Object.prototype.hasOwnProperty.call(camCollegeColor, club)) {
        return camCollegeColor[club];
    }
    if (typeof(event) != "undefined") {
        const club_abbreviation = abbreviateCrew(club, event);
        if (Object.prototype.hasOwnProperty.call(camCollegeColor, club_abbreviation)) {
            return camCollegeColor[club_abbreviation];
        }
    }
    if (Object.prototype.hasOwnProperty.call(townColor, club)) {
        return townColor[club];
    }
    if (Object.prototype.hasOwnProperty.call(oxCollegeColor, club)) {
    ...
    for (let dayNum = 0; dayNum < event.days; dayNum++) {
        for (let crew = 0; crew < crewsFirstDay.length; crew++) {
            if (dayNum === 0) {
                ...
                move[dayNum][division][positionInDivision] = +crewsFirstDay[crew]['Start position'] -
                ...
                move[dayNum][division][positionInDivision] = +crewsAllDays[event.days * crew + dayNum - 1].Position -
                    +crewsAllDays[event.days * crew + dayNum].Position;
